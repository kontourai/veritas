...
function readExceptionCount(rootDir, evidenceRecord) {
  ...
  const fileCount = existsSync(exceptionPath)
    ...
    : 0;
  return fileCount + (evidenceRecord.exceptions?.length ?? 0);
}
...
export function buildFilesystemStandardsFeedbackDraft({ rootDir, evidenceRecord, evidencePath, churnThreshold = 0.3 }) {
  ...
  return {
    ...
    authority_settings_id: 'filesystem-observed',
    mode: 'observe',
    ...
    prefilled_measurements: {
      ...
      exception_count: readExceptionCount(rootDir, evidenceRecord),
    ...
    prefilled_sources: {
      ...
      exception_count: 'filesystem-inferred',
    },
    files: evidenceRecord.files ?? [],
    notes: ['Derived from filesystem artifacts because no session log reader was available.'],
    missing_confirmation_fields: ['required_followup'],
  };
}

export function observeFilesystemStandardsFeedback({ rootDir, evidencePath, outputPath, churnThreshold = 0.3 }) {
  if (!evidencePath) {
    throw new Error('veritas feedback observe --tool none requires --evidence <path>');
  }
  const resolvedEvidencePath = resolve(rootDir, evidencePath);
  const evidenceRecord = loadEvidenceArtifact(resolvedEvidencePath);
  const draft = buildFilesystemStandardsFeedbackDraft({
    rootDir,
    evidenceRecord,
