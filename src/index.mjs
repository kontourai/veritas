...
export function loadMarkerBenchmarkScenario(scenarioPath) {
  return JSON.parse(readFileSync(scenarioPath, 'utf8'));
}

export function loadMarkerBenchmarkTranscript(transcriptPath) {
  return JSON.parse(readFileSync(transcriptPath, 'utf8'));
}

export function loadMarkerBenchmarkSuite(suitePath) {
...
export function parseMarkerEvalArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (token === '--without-veritas-transcript') {
      options.withoutVeritasTranscriptPath = argv[index + 1];
      index += 1;
      continue;
    }
    if (token === '--with-veritas-transcript') {
      options.withVeritasTranscriptPath = argv[index + 1];
      index += 1;
    }
  }
