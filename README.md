...
`print codex-hook` and `apply codex-hook` are the first runtime-specific adapter layer on top of that generic hook. They generate a tracked `.ai-guidance/runtime/codex-hooks.json` snippet and can explicitly merge it into either a chosen Codex `hooks.json` file or a chosen Codex home via `--codex-home`, without silently mutating global config by default.
